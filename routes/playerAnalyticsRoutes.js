...
// Player performance analytics: trend (per match) + opponent summary
// Uses player_performance + union of match_history + test_match_results
const express = require('express');
const router = express.Router();
const pool = require('../db');

// Utility: safe uppercase for MT
const normType = v => String(v || 'ALL').toUpperCase();

/**
 * GET /api/players/trend
 * ?player=Ranaj%20Parida&type=ALL|ODI|T20|TEST&opponent=<team or ALL>&metric=runs|batting_avg|strike_rate|wickets|bowling_avg
 * Returns a chronological series per match with the metric and a 5-match moving average (ma5).
 */
router.get('/trend', async (req, res) => {
  try {
    ...
    // Union matches from both tables so we can compute opponent from teams
    const seriesSql = `
      WITH m AS (
        SELECT match_name, LOWER(TRIM(match_type)) AS mt, team1, team2, created_at
        FROM match_history
        UNION ALL
        SELECT match_name, 'test' AS mt, team1, team2, created_at
      ...
      SELECT
        ...
        opponent,
        metric_value,
        -- 5-match moving average of the selected metric
    ...
    console.error('❌ /api/players/trend error:', e);
    res.status(500).json({ error: 'Server error' });
  }
});

/**
 * GET /api/players/opponent-summary
 * ?player=Ranaj&type=ALL|ODI|T20|TEST
 * Aggregates per opponent for the player (runs, SR, batting avg, wickets, bowl avg)
 * Also returns overall aggregates to help build suggestions on the client.
 */
router.get('/opponent-summary', async (req, res) => {
  try {
    ...
    const sql = `
      WITH m AS (
        SELECT match_name, LOWER(TRIM(match_type)) AS mt, team1, team2
        FROM match_history
        UNION ALL
        SELECT match_name, 'test' AS mt, team1, team2
        FROM test_match_results
      ),
      raw AS (
        SELECT
    ...
    // Overall aggregates (for suggestions)
    const overall = r.rows.reduce((acc, x) => {
      acc.runs += Number(x.runs || 0);
      acc.balls_faced += Number(x.balls_faced || 0);
      acc.wickets += Number(x.wickets || 0);
      acc.runs_given += Number(x.runs_given || 0);
      acc.outs += Number(x.outs || 0);
      return acc;
    ...
    console.error('❌ /api/players/opponent-summary error:', e);
    res.status(500).json({ error: 'Server error' });
  }
});
