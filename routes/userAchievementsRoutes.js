// ✅ FIXED: Ensure matchTypeFilter and params are used properly across queries
// ✅ FIXED: Use ILIKE and dynamic query param consistency
// ✅ FIXED: Sum across all rows per player (e.g., for multiple entries like Ranaj Parida)
...
router.get('/', async (req, res) => {
  try {
    ...
    // 🎯 Create fresh scoped filters per query to avoid mismatch
    const filterClause = matchType !== 'All' ? `AND pp.match_type ILIKE $2` : '';
   ...
   const runScorerQuery = `
      ...
      LIMIT 1
    `;
    const { rows: runRows } = await pool.query(runScorerQuery, filterParams);
    ...
    const centuriesQuery = `
      ...
      LIMIT 1
    `;
    const { rows: centuriesRows } = await pool.query(centuriesQuery, filterParams);
    ...
    const wicketsQuery = `
      ...
      LIMIT 1
    `;
    const { rows: wicketsRows } = await pool.query(wicketsQuery, filterParams);
    ...
    if (matchType === 'Test') {
      const testMostWinsQuery = `
        ...
        FROM (
          SELECT team1 AS team_name FROM test_match_results
          WHERE user_id = $1 AND LOWER(TRIM(winner)) = LOWER(TRIM(team1))
          UNION ALL
          SELECT team2 AS team_name FROM test_match_results
